...
export default function UpcomingIPOs() {
  ...
  const fetchIPOs = async () => {
    try {
      ...
      // Only show upcoming or discovered
      setIpos(res.data.filter(i => ['UPCOMING', 'DISCOVERED'].includes(i.lifecycle_status)));
    } catch (err) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Upcoming IPOs</h1>
          <p className="text-sm text-gray-400 mt-1">{filtered.length} upcoming offerings</p>
        </div>
        <button onClick={runDiscovery} disabled={loading} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white text-sm font-medium hover:bg-primary-hover transition-colors disabled:opacity-50">
      ...
      {!loading && filtered.length === 0 && (
        <div className="text-center py-16 glass-card">
          <CalendarRange className="w-12 h-12 mx-auto mb-4 text-gray-600" />
          <p className="text-gray-400">No upcoming IPOs found.</p>
          <button onClick={runDiscovery} className="mt-4 px-4 py-2 rounded-lg bg-primary text-white text-sm">
            Run Discovery
          </button>
        </div>
      )}
